...
fallback level (0 = full kernels, 1 = night kernels dropped, 2 = all weather dropped, 3 =
day-type widened + inventory dropped),
effective sample size, the target context it compared against (`basis.target`, including
`startBikes` — tonight's 10pm inventory; `sweptEvening` — bikes trucked out 5–10pm tonight;
`nightPrecipMm`/`nightDewC` — tonight's 8pm–2am rain and 10pm dew point, forecast past 10pm),
and the top contributing days with their weights.
```json
...
- `POST /admin/run?push=0|1` — run the nightly pipeline now, bypassing the 10pm guard. `push`
...
Both `0 2 * * *` and `0 3 * * *` UTC fire; only the one landing at **10pm America/Toronto** runs
...
10pm still produces a prediction from existing facts.
